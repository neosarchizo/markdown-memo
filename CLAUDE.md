...
안드로이드 emulator를 실행해놓을테니 각 단계마다 expo-mcp로 확인해줘.
expo-mcp를 사용하다가 스크린샷을 저장할때 꼭 필요한 경우가 아니라면 메모리에 불러오지 말고,
부득이하게 불러온 경우에는 세션을 재시작해야한다고 알려줘
작업을 완료하면 git을 통해 저장해줘

앱 테스트 할때는 `npm start` 명령어를 실행해서 Expo 서버를 실행한 뒤에, expo-mcp로 확인해줘